import * as React from 'react';

import Background from './Background'
import NavBar from './NavBar'
// import { a } from '../components/a';
import SvgIcon from '@mui/material/SvgIcon';
import InstagramIcon from '@mui/icons-material/Instagram';
import LinkedInIcon from '@mui/icons-material/LinkedIn';
import GitHubIcon from '@mui/icons-material/GitHub';
import * as Quote from 'inspirational-quotes';

function HeartBeat() {
  return (
      <div class="heart-rate">
        <svg version="1.0" xmlns="http://www.w3.org/2000/svg" 
           x="0px" y="0px" 
          width="150px" height="73px" viewBox="0 0 150 73" 
          enable-background="new 0 0 150 73" 
          >
          <polyline fill="none" stroke="#009B9E" stroke-width="3" 
            stroke-miterlimit="10" points="0,45.486 38.514,45.486 44.595,33.324 50.676,45.486 57.771,45.486 62.838,55.622 71.959,9 80.067,63.729 84.122,45.486 97.297,45.486 103.379,40.419 110.473,45.486 150,45.486"
          />
        </svg>
      <div class="fade-in"/>
      <div class="fade-out"/>
    </div>
  )
}

export function PageTemplate({children, className}) {
   const [isHeartBeatDone, setIsHeartBeatDone] = React.useState(false);
   React.useEffect(() => {
    setTimeout(() => {
      setIsHeartBeatDone(true);
    }, 1000)
   }, [])
   return(
    <main id="main" className="flex flex-col">
        {!isHeartBeatDone && <div className="h-screen flex items-center justify-center"><HeartBeat/></div>}
        {isHeartBeatDone && (<>
            <NavBar/>
            <Background className={`grow text-white w-full ${className}`}>
                {children}
            </Background>
            <div className="flex py-2 text-white justify-center">
                <a href="https://github.com/htruong1" target="_blank" className="p-4">
                  <SvgIcon
                    fontSize="large"
                  >
                    <GitHubIcon/>
                  </SvgIcon>
                </a>
                <a href="https://www.instagram.com/forever_wei_ting/" target="_blank" className="p-4">
                  <SvgIcon
                      fontSize="large"
                    >
                      <InstagramIcon/>
                    </SvgIcon>
                </a>
                <a href="https://www.linkedin.com/in/htruong1/" target="_blank" className="p-4">
                <SvgIcon
                    fontSize="large"
                  >
                    <LinkedInIcon/>
                  </SvgIcon>
                </a>
              </div>
            </>)}
      </main>
    )
}